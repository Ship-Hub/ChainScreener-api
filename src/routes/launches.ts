...
export async function registerLaunchRoutes(app: FastifyInstance) {
  ...
  app.get("/api/launches", async (request) => {
    const query = request.query as { chain?: string; sort?: string; platform?: string };
    ...
    const tokens = await listMarketTokens(chain, sort, platform);
    return { data: tokens };
  });
  ...
  app.get("/api/launches/by-platform", async () => {
    ...
    const results = await Promise.all(
      platformsWithAddresses.map(async (p) => {
        const tokens = await listMarketTokens(p.chain, "newest", p.key);
